...
cd ../datasets
datalad install https://github.com/OpenNeuroDatasets/ds003787.git

# Download the required freesurfer data
cd ds003787/derivatives/freesurfer
    ...
    if [ ${subject:0:3} != "sub" ]; then
        ...
        do
            ...
            do
                ...
                mris_convert -c ../prf-estimation/$subject/ses-nyu3t01/"$subject"_ses-all_task-logbar_hemi-"$hemi"_space-fsnative_prf/"$subject"_ses-all_task-logbar_hemi-"$hemi"_space-fsnative_"$metric".mgz $subject/surf/"$hemisphere".white \
                ../prf-estimation/$subject/ses-nyu3t01/"$subject"_ses-all_task-logbar_hemi-"$hemi"_space-fsnative_prf/"$subject"_ses-all_task-logbar_hemi-"$hemi"_space-fsnative_"$metric".gii \

                echo "Resampling native data to fsaverage space..."
                wb_command -metric-resample ../prf-estimation/$subject/ses-nyu3t01/"$subject"_ses-all_task-logbar_hemi-"$hemi"_space-fsnative_prf/"$subject"_ses-all_task-logbar_hemi-"$hemi"_space-fsnative_"$metric".gii \
